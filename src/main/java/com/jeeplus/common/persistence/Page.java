...
public class Page<T> {
	...
	public Page(HttpServletRequest request, HttpServletResponse response, boolean flag){
		...
		if (StringUtils.isNumeric(size)){
			CookieUtils.setCookie(response, "limit", size);
			this.setPageSize(Integer.parseInt(size));
		}else if (request.getParameter("repage")!=null){
			no = CookieUtils.getCookie(request, "limit");
			if (StringUtils.isNumeric(size)){
				this.setPageSize(Integer.parseInt(size));
			}
		}
...
//		sb.append("</div>");
		...
		sb.append("<li class=\""+getSelected(pageSize,2)+ "\"><a href=\"javascript:"+funcName+"("+pageNo+",2,'"+funcParam+"');\">2</a></li>");
		sb.append("<li class=\""+getSelected(pageSize,5)+ "\"><a href=\"javascript:"+funcName+"("+pageNo+",5,'"+funcParam+"');\">5</a></li>");
